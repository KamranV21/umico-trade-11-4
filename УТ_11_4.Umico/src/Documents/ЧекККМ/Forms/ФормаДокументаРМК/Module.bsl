...
Процедура UMC_ПробитьЧекЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ВыполненаОперацияНаУстройстве
		И Результат.ИзмененныеДанныеЗаписаны
		И ЗначениеЗаполнено(Объект.UMC_НомерСчетаUmico) Тогда
			
		UMC_ПередатьИнформациюОЧекеВUmico();	
...
Функция UMC_ПередатьИнформациюОЧекеВUmico()
	...
	UMC_ОбработкаОбменаСUmico.ВариантИспользования = 
		Перечисления.UMC_ВариантыИспользованияПодключенияКUmico.ДляРозничныхПродаж;
