...
Процедура ПолучитьСтатусОбменаСUmico(Форма, Документ) Экспорт
	...
	Если Выборка.Следующий() Тогда
		Если НРег(Выборка.Статус) = "paid" Тогда
			Форма.UMC_СтатусПередачиВUmico = НСтр("ru = 'Документ успешно передан в Umico'");
			Форма.Элементы.UMC_СтатусПередачиВUmico.ЦветТекста = ЦветаСтиля.ЦветАкцента;	
		Иначе
			Форма.UMC_СтатусПередачиВUmico = СтрШаблон(НСтр("ru = 'Документ был передан в Umico. Но процесс не завершен: %1'"), Выборка.Статус);
