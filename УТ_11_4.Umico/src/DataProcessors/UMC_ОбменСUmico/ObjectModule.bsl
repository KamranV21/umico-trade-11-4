...
//  см. UMC_ОбменСUmico.НоваяСтруктураСведенияОСчетеUmico.
...
Функция ЗарегистрироватьЧек(Документ) Экспорт
	...
	Ответ = UMC_ОбменСUmico.НоваяСтруктураРезультатРегистрацииЧекаВUmcio();
	...
	УдалитьСведенияОЧекеUmico(Документ, ЭтоПродажа);
	...
	Пока КоличествоПопыток > 0 Цикл
		...
		Ответ = ПроверитьСтатусЧекаUmico(Документ, ЭтоПродажа, Идентификатор);
		Если Ответ <> Неопределено И Не Ответ.Ожидание Тогда
			Возврат Ответ;
		КонецЕсли;
...
//	HTTPСоединение = Новый HTTPСоединение("umico-integration-api-stage.umico.az", , , , , , Новый ЗащищенноеСоединениеOpenSSL);
...
Функция ОбработатьОтветПоСведениямОСчете(СтруктураОтвета)
	...
	СведенияОСчете = UMC_ОбменСUmico.НоваяСтруктураСведенияОСчетеUmico();
...
Функция ОбработатьОтветПоСтатусуЧека(СтруктураОтвета)
...
Функция ЗарегистрироватьЧекВозвратаВUmico(Документ, ДокументОснование, ИдентификаторЧекаПродажи,
	...
	Ресурс = "/" + ПолучитьИмяРесурсаПоВидуОперации(Истина) + "/" + ?(ИдентификаторЧека = Неопределено, "", "accept");
...
Функция ПроверитьСтатусЧекаUmico(Документ, ЭтоПродажа, Идентификатор)
	...
	Возврат ОбработатьОтветПоСтатусуЧека(СтруктураОтвета);

КонецФункции

// Удалить сведения о чеке Umico.
// 
// Параметры:
//  Документ - ДокументСсылка.ЧекККМ, ДокументСсылка.РеализацияТоваровУслуг, ДокументСсылка.ВозвратТоваровОтКлиента, ДокументСсылка.ЧекККМВозврат - Документ
//	ЭтоПродажа - Булево
Процедура УдалитьСведенияОЧекеUmico(Документ, ЭтоПродажа)
	...
	Ресурс = "/" + ПолучитьИмяРесурсаПоВидуОперации(ЭтоПродажа) + "/" + Идентификатор;
	ВыполнитьHTTPЗапрос("delete", "DELETE", Ресурс);
