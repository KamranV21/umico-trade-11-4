...
Процедура ПолучитьСведения(Команда)
	...
	ПолучитьСведенияОСчете();
	УстановитьВидимость();

	Если ПолученыСведенияПоСчету = Истина Тогда
		ПерейтиКЭтапуВводаСуммы();
	КонецЕсли;
...
&НаСервере
Процедура ПолучитьСведенияОСчете()

	Значение = РеквизитФормыВЗначение("Объект");

	НомерСчетаБезПробелов = СтрЗаменить(НомерСчета, " ", "");

	СведенияОСчете = Значение.ПолучитьСведенияОСчете(НомерСчетаБезПробелов);

	Если СведенияОСчете = Неопределено Тогда
		ПолученыСведенияПоСчету = Ложь;
		ОстатокБаллов = 0;
	Иначе
...
Процедура УстановитьВидимость()
	...
	Если ПолученыСведенияПоСчету = Неопределено Тогда
		Элементы.ИнформационнаяНадпись.Заголовок = НСтр(
			"ru = 'Введите номер счета, чтобы получить сведения о балансе.'");
...
&НаСервере
Процедура ПерейтиКЭтапуВводаСуммы()
