...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Чек будет отменен в Umico. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПослеЗакрытияВопроса(Результат, ДополнительныеПараметры) Экспорт
	...
	Ответ = ОтменитьЧекВUmico(Документ);
	Если Ответ.Результат Тогда

		ТекстСообщения = НСтр("ru = 'Чек успешно отменен в Umico.'");
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Ответ.ТекстОшибки) Тогда
			ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Не удалось отменить чек в Umico по причине - %1.'"), Ответ.ТекстОшибки);
		Иначе
			ТекстСообщения = НСтр("ru = 'Не удалось отменить чек в Umico.'");
		КонецЕсли;
...
&НаСервере
Функция ОтменитьЧекВUmico(Документ)
	...
	Если ОбработкаОбменаСUmico.ОпределитьПараметрыПодключения() Тогда
		Возврат ОбработкаОбменаСUmico.ОтменитьЧек(Документ);
	Иначе
