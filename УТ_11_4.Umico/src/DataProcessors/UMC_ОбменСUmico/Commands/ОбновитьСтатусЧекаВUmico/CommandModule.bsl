...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Будет получен статус чека в Umico. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПослеЗакрытияВопроса(Результат, ДополнительныеПараметры) Экспорт
	...
	Ответ = ОбновитьСтатусЧекаВUmico(Документ);
	Если Ответ.Результат Тогда

		ТекстСообщения = НСтр("ru = 'Статус чека успешно обновлен.'");
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Ответ.ТекстОшибки) Тогда
			ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Не удалось обновить статус чека по причине - %1.'"), Ответ.ТекстОшибки);
		Иначе
			ТекстСообщения = НСтр("ru = 'Не удалось обновить статус чека в Umico.'");
		КонецЕсли;
...
&НаСервере
Функция ОбновитьСтатусЧекаВUmico(Документ)
	...
	Если ОбработкаОбменаСUmico.ОпределитьПараметрыПодключения() Тогда
		Возврат ОбработкаОбменаСUmico.ПолучитьСтатусЧека(Документ);
	Иначе
